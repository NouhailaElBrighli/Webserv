...
/*   Updated: 2023/06/04 11:55:00 by hsaidi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Delete::delete_file()
{
	...
	bool   is_found = false;
	 for (std::vector<ConfigLocationParser*>::const_iterator it = this->config_location_parser.begin();
		...
		file_name = this->main_client->get_request("Request-URI");

		if ((*it)->get_location().find("cgi") != string::npos)
			continue;
		if (this->main_client->get_request("Request-URI").find((*it)->get_location()) != string::npos) 
		{
			cout << "++ location ++" << endl;
			file_name.erase(0, (*it)->get_location().length());
			is_found = true;
		} 
		else if (this->main_client->get_request("Request-URI").find((*it)->get_root()) != string::npos) {
